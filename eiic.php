...
function comments_img_embed($comment) {
  ...
  $comment = preg_replace(array('#(http://([^\s]*)\.(jpg|gif|png|JPG|GIF|PNG))#','#(https://([^\s]*)\.(jpg|gif|png|JPG|GIF|PNG))#'),'<a rel="nofollow" href="$1"><img src="$1" alt="" width="'.$size.'" height="" /></a>', $comment);
  return $comment;
}
?>